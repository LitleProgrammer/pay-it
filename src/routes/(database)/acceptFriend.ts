...
export async function acceptFriend(userID: any, friendName: any): Promise<string> {
    connect();
    ...
    if (friend) {
        if (!friend.userID !== userID) {
            ...
            if(!result) {
                usersCollection.updateOne({userID: userID}, {$push:{friends: friend.userID}});
                usersCollection.updateOne({userID: userID}, {$pull:{incomingFriends: friend.userID}});
                usersCollection.updateOne({userID: friend.userID}, {$push:{friends: userID}});
